...
func GetLikes(PostID uint64) (*[]api.LikeApi, error) {
	...
	res := orm.Engine.Table("likes").Select("like.id,users.full_name,users.id").Joins("JOIN posts on posts.id = likes.post_id").Joins("JOIN users on likes.user_id = users.id").Scan(&likes)
	if res.Error != nil {
